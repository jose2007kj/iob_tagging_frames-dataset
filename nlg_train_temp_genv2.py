import json
import re
with open('nlg_train_tempv2-test.txt', 'w') as f:
				...
				if 'wizard' in item['author']:
					...
					for args in item['labels']['acts']:
						...
						for val in args['args']:
							...
							wc = 0 
							if val['key'] == 'ref':
								print("list### inside ref")
								# print()
								print("intent",intent)
								if 'val' in val:
									for ref in val['val']:
										print(type(ref))
										for anot in ref['annotations']:
											if 'key' in anot and 'val' in anot:
												intent+=" "+"ref+"+anot['key']
												if type(anot['val'])==str:
													nlg_temp=re.sub(anot['val'],"ref+"+anot['key'], nlg_temp)
												else:
													nlg_temp=re.sub(str(anot['val']),"ref+"+anot['key'], nlg_temp)
							...
							# nlg temp
								...
								if val['key'] != 'id':
									...
									if 'val' in val:
										...
										if val['key'] !='action':
											if type(val['val'])==str:
												nlg_temp=re.sub(val['val'],val['key'], nlg_temp)
											else:
												nlg_temp=re.sub(str(val['val']),val['key'], nlg_temp)
					removed = re.sub(r",\s+", " ", nlg_temp)#for removing hello, I am also taking care of 23,000
					dot = re.sub(r"\.+", " ", removed)#for removing one or more dots
					...
					exc =exc.rstrip()
					pattern = re.compile(r'\s+')
					final = re.sub(pattern," ", exc)
					nlg_temp= final.replace('$',"")
					nlg_temp= nlg_temp.lower()
					split_temp_keys=intent.split()
					print("before setting",split_temp_keys)
					# split_temp_keys=list(set(split_temp_keys))
					print("after setting",split_temp_keys)
					# sorted_temp_key=sorted(split_temp_keys)
					intent =''
					for i,temp_w in enumerate(split_temp_keys):
						if i == 0:
							intent+=temp_w
						else:
							intent+=' '+temp_w
