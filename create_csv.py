...
with open('dontdont_template.txt', 'w') as f:
							...
							if type(val['val']) == str:
								...
								for idx,word in enumerate(final.split()):
									if val['key'].lower() in word:
										...
										if "flex" in word:
											if "no "+word in final:
												final = re.sub("no "+word,'$'+val['key']+'$', final, 1)
												...
												final = re.sub("not"+word,'$'+val['key']+'$', final, 1)
												if args['name'] not in ontology:
													ontology[args['name']] = {}
												if val['key'] not in ontology[args['name']]:
													ontology[args['name']][val['key']] = {}
												...
												final = re.sub(word,'$'+val['key']+'$', final, 1)
												if args['name'] not in ontology:
													ontology[args['name']] = {}
												if val['key'] not in ontology[args['name']]:
													ontology[args['name']][val['key']] = {}
										...
										# 		final = final.replace(word,'$'+val['key']+'$')
											...
											if "free "+word in final:
												# final = final.replace("free "+word,'$'+val['key']+'$')
												final = re.sub("free "+word,'$'+val['key']+'$', final, 1)
												if args['name'] not in ontology:
													ontology[args['name']] = {}
												if val['key'] not in ontology[args['name']]:
													ontology[args['name']][val['key']] = {}
							...
							if wc == 1:
								...
								for idx,word in enumerate(final.split()):
									if n_val in word:
										pos = idx
										print("position brek",pos)
										final = re.sub(n_val,'$'+val['key']+'$', final, 1)
										# final = final.replace(word,'$'+val['key']+'$')
										slots.append(val['key'])
										act.append(args['name'])
										if args['name'] not in ontology:
